...
import { DocsUploadButton } from "@/components/DocsUploadButton";
...
export function ChatPageContent({ chatId }: ChatPageContentProps) {
  ...
  useEffect(() => {
    if (!isLoading && profile && !(profile as any).openai_api_key) {
  ...
  if (!(profile as any)?.openai_api_key) {
    return (
      <div className="h-screen flex items-center justify-center animated-bg">
        <motion.div
        ...
        >
          ...
          >
            To get started, you'll need to configure your OpenAI API key. This
            allows you to chat with OpenAI models.
          </motion.p>
  ...
  return (
    ...
    >
      ...
      >
        {/* Upload Docs button in top-right of chat column */}
